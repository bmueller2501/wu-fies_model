...
print("BAY: Start with model-bayes_ssvs.R!")
try(rji$sendMessage("BAY: Start with model-bayes_ssvs.R!"))
...
# load prm from model_randomforest_1_wigeo.R
prm <- read.csv(file.path(path_data, "bayes_ssvs/prm-bayes_ssvs.csv"))

mod.prm <- prm[which.max(prm$mod.r2), ]
sev.prm <- prm[which.max(prm$sev.r2), ]

try(rji$sendMessage("BAY: Start with re-run CV for best hyperparameters!"))
...
for (iso3 in unique(moddat$ISO3)){
  ...
  print(paste0("BAY mod: ", round(sum(!is.na(moddat$fies.mod.pred.cv))/nrow(moddat)*100, 2), "%"))
  try(rji$sendMessage(paste0("BAY mod: ", round(sum(!is.na(moddat$fies.mod.pred.cv))/nrow(moddat)*100, 2), "%")))
  ...
  for (i in 2:nrow(mdf)){
    moddat$fies.mod.pred.cv[!ix] <- moddat$fies.mod.pred.cv[!ix] + moddat[!ix, mdf$term[i]]*mdf[["mean"]][i]
  }
...
for (iso3 in unique(moddat$ISO3)){
  ...
  for (i in 2:nrow(mdf)){
    moddat$fies.sev.pred.cv[!ix] <- moddat$fies.sev.pred.cv[!ix] + moddat[!ix, sdf$term[i]]*sdf[["mean"]][i]
  }
...
## run full models -------------------------------------------------------------
print("BAY: Start with run full models with best parameters!")
try(rji$sendMessage("BAY: Start with run full models with best parameters!"))
...
y <- as.matrix(moddat %>% select(fies.mod.logit))
bigX <- cbind(matrix(1, nrow = nrow(moddat), ncol = 1), as.matrix(moddat %>% select(vars)))
colnames(bigX) <- c("(Intercept)", vars)
...
y <- as.matrix(moddat %>% select(fies.sev.logit))
bigX <- cbind(matrix(1, nrow = nrow(moddat), ncol = 1), as.matrix(moddat %>% select(vars)))
colnames(bigX) <- c("(Intercept)", vars)
nburn <- 100000         #number of burn ins
nsave <- 900000         #number of saved draws
...
# project and apply inverse logit transformation
moddat$fies.mod.pred <- mdf[["mean"]][mdf$term == "(Intercept)"]
moddat$fies.sev.pred <- sdf[["mean"]][sdf$term == "(Intercept)"]
# preddat$fies.mod.pred <- mdf[["mean"]][mdf$term == "(Intercept)"]
# preddat$fies.sev.pred <- sdf[["mean"]][sdf$term == "(Intercept)"]
for (i in 2:nrow(mdf)){
  moddat$fies.mod.pred <- moddat$fies.mod.pred + moddat[ , mdf$term[i]]*mdf[["mean"]][i]
  moddat$fies.sev.pred <- moddat$fies.sev.pred + moddat[ , sdf$term[i]]*sdf[["mean"]][i]
  # preddat$fies.mod.pred <- preddat$fies.mod.pred + preddat[ , mdf$term[i]]*mdf[["mean"]][i]
  # preddat$fies.sev.pred <- preddat$fies.sev.pred + preddat[ , sdf$term[i]]*sdf[["mean"]][i]
}
...
# write.csv(moddat, file.path(path_data, "bayes_ssvs/moddat-bayes_ssvs.csv"), row.names = F)
# write.csv(preddat, file.path(path_data, "bayes_ssvs/preddat-bayes_ssvs.csv"), row.names = F)

print("BAY: Done with model-bayes_ssvs.R!")
try(rji$sendMessage("BAY: Done with model-bayes_ssvs.R!"))
